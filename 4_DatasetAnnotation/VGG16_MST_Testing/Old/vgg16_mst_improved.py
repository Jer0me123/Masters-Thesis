...
def build_dataloaders(csv_path, image_dir, batch_size, val_ratio,
    ...
    # ---------- CLASS WEIGHTS (UNCAPPED for better minority class handling) ----------
    ...
    print("[INFO] Class weights (inverse frequency, UNCAPPED):")
    for k in range(num_classes):
        print(f"  Class {k} (MST {k+1}): weight={raw_weights[k]:.3f}")
    print()

    # Build weight tensor for loss functions
    class_weight_tensor = torch.zeros(num_classes, dtype=torch.float32)
    for k in range(num_classes):
        class_weight_tensor[k] = raw_weights.get(k, 1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Improved VGG16 MST Classifier")
...
# python vgg16_mst_improved.py --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
# --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
# --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\ImprovedTest" ^
# --gpu 0 ^
# --input-mode "rgb" ^
