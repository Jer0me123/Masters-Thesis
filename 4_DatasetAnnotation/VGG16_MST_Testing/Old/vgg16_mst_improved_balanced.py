# ##############################################################
# #  VGG16 MST CLASSIFIER - IMPROVED GENERALIZATION VERSION
# #  
# #  Key Improvements:
# #  - Label smoothing for better generalization
# #  - Enhanced augmentation (color-preserving)
# #  - Gradual layer unfreezing
# #  - Multi-task learning (MST + 3-bin)
# #  - Better class balancing with uncapped weights
# #  - Supervised contrastive loss
# #  - Comprehensive diagnostic tools
# #  - Dataset balancing with per-class image cap
# ##############################################################
...
#     """
#     Combines Focal Loss with Label Smoothing for better generalization.
...
#     Modified for ordinal labels (closer MST values = more similar)
#     """
...
# ##############################################################
# # 5. VGG16 MST Classifier with Multi-Task Learning
# ##############################################################

# class VGG16MSTMultiTask(nn.Module):
#     """
#     VGG16 with multi-task learning:
#     - Main task: 10-class MST classification
#     - Auxiliary task: 3-bin classification (light/mid/dark)
#     - Contrastive learning: feature similarity for same MST labels
#     """
...
#         # Main MST classifier (10 classes)
#         self.mst_classifier = nn.Linear(512, 10)

#         # Auxiliary 3-bin classifier
...
#         for m in [self.feature_extractor, self.mst_classifier, 
#                   self.bin_classifier, self.projection]:
...
#             return mst_logits, bin_logits, proj
...
# def train_one_epoch(model, loader, mst_criterion, bin_criterion, contrastive_criterion,
#                     optimizer, device, epoch, total_epochs, label_mode,
#                     use_contrastive=True, lambda_bin=0.3, lambda_contrast=0.1):
#     """
#     Multi-task training with:
#     - Main MST classification loss
#     - Auxiliary 3-bin classification loss
#     - Supervised contrastive loss
#     """
#     model.train()
...
#         mst_logits, bin_logits, features = model(imgs, return_features=True)

#         # Main MST loss
#         mst_loss = mst_criterion(mst_logits, labels)

#         # Auxiliary 3-bin loss
#         bin_labels = torch.tensor([tone_bin_from_class_idx(l.item(), label_mode) 
...
#         # Combined loss
#         total_loss = mst_loss + lambda_bin * bin_loss + lambda_contrast * contrast_loss
...
#         running_bin_loss += bin_loss.item() * batch_size
...
#     avg_bin_loss = running_bin_loss / total_samples
...
# def validate(model, loader, mst_criterion, bin_criterion, device, epoch, label_mode,
#              lambda_bin=0.3, analyze=False):
#     """
#     Validation with both image-level and person-level metrics
#     """
#     model.eval()
...
#             mst_logits, bin_logits = model(imgs, return_features=False)
...
#             total_loss = mst_loss + lambda_bin * bin_loss
...
# def main():
#     parser = argparse.ArgumentParser(description="Improved VGG16 MST Classifier with Dataset Balancing")
...
#     parser.add_argument("--lambda-bin", type=float, default=0.3,
#                         help="Weight for auxiliary 3-bin classification loss")
...
#     print("\n" + "="*70)
#     print("VGG16 MST CLASSIFIER - IMPROVED GENERALIZATION")
#     print("="*70)
...
#     print(f"[INFO] Multi-task weights: bin={args.lambda_bin}, contrast={args.lambda_contrast}")
...
#     # Main MST loss with label smoothing
...
#     # Auxiliary 3-bin loss (no label smoothing, simpler task)
...
#             model, train_loader,
#             mst_criterion, bin_criterion, contrastive_criterion,
...
#               f"Bin={train_results['bin_loss']:.4f}, "
...
#               f"(MST={val_results['mst_loss']:.4f}, "
#               f"Bin={val_results['bin_loss']:.4f})")
...
# python vgg16_mst_improved_balanced.py --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
# --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
# --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\ImprovedTest_Balanced" ^
# --gpu 0 ^
# --input-mode "rgb" ^
# --max-per-class 5000 ^
...
# python vgg16_mst_improved_balanced.py ^
#   --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
#   --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
#   --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\BalancedV2" ^
#   --gpu 0 ^
#   --input-mode rgb ^
#   --use-bn ^
#   --max-per-class 5000 ^
...
#   --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
#   --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
#   --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\BalancedV3_500" ^
#   --gpu 0 ^
#   --input-mode rgb ^
#   --use-bn ^
#   --max-per-class 500 ^
...
#   --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
#   --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
#   --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\ClaudeConfig_1" ^
#   --gpu 0 ^
#   --input-mode rgb ^
#   --use-bn ^
#   --max-per-class 3000 ^
...
#  VGG16 MST CLASSIFIER - IMPROVED GENERALIZATION VERSION
#  
#  *** FIXED VERSION - NaN Issues Resolved ***
#  
#  Bug Fixes Applied:
#  1. LabelSmoothingFocalLoss (line ~728):
#     - Changed torch.exp(log_probs) to F.softmax(logits, dim=1)
#     - Added clamping for pt values (min=1e-7, max=1-1e-7)
#     - Added clamping for focal_weight (max=100.0)
#     - Added NaN/Inf safety check with fallback to cross-entropy
#  
#  2. SupervisedContrastiveLoss (line ~766):
#     - Increased temperature from 0.07 to 0.5 for stability
#     - Added clamping for similarity matrix (-10 to 10)
#  
#  These fixes prevent numerical overflow that caused NaN losses.
#  
#  Key Improvements:
#  - Label smoothing for better generalization
#  - Enhanced augmentation (color-preserving)
...
#  - Multi-task learning (MST + 3-bin)
#  - Better class balancing with uncapped weights
#  - Supervised contrastive loss (now stable!)
#  - Comprehensive diagnostic tools
#  - Dataset balancing with per-class image cap
##############################################################
...
class LabelSmoothingFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        log_probs = F.log_softmax(logits, dim=1)
        probs = F.softmax(logits, dim=1)  # FIXED: was torch.exp(log_probs)
...
class SupervisedContrastiveLoss(nn.Module):
    ...
    Modified for ordinal labels (closer MST values = more similar)
    """
    def __init__(self, temperature=0.5, base_temperature=0.5):  # FIXED: was 0.07):
        super().__init__()
        self.temperature = temperature
        self.base_temperature = base_temperature
...
# Overall Metrics:
#   Top-1 Accuracy: 28.07%
#   Off-by-1 Accuracy: 59.40%
#   3-Bin Accuracy: 62.33%
#   L2-LAB Distance: 18.567 ± 18.663
# python vgg16_mst_improved_balanced.py ^
#   --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
#   --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
#   --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\Fixed_Stable" ^
#   --gpu 0 ^
#   --input-mode rgb ^
#   --use-bn ^
#   --max-per-class 3000 ^
...
#   --csv-path "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2\annotations.csv" ^
#   --image-dir "G:\Thesis\CasualConversationv2_Dataset\Segmented_CCV2" ^
#   --save-dir "G:\Thesis\CasualConversationv2_Dataset\Models\MoreData" ^
#   --gpu 0 ^
#   --input-mode rgb ^
#   --use-bn ^
#   --max-per-class 8000 ^
