...
class SkinTonePredictor:
    """
    Wrapper for VGG16 MST models (classifier or regressor)
    """
    ...
    def __init__(
        ...
        print(f"[SkinTone] Loading {model_type} model from {model_path}")
        print(f"[SkinTone] Output mode: {output_mode}, Input mode: {input_mode}")
        
        # Load model
        if model_type == "classifier":
            num_classes = 3 if output_mode == "binned" else 10
        ...
        print(f"[SkinTone] Model loaded on {self.device}")
    
    def predict(self, img_pil: Image.Image) -> Tuple[Optional[int], Optional[int]]:
        """
        Returns: (mst_label, bin_label)
        - mst_label: 1-10 for unbinned, 0-2 for binned
        - bin_label: 0-2 always
        """
        try:
            ...
            with torch.no_grad():
                if self.model_type == "classifier":
                    ...
                    if self.output_mode == "binned":
                        # Model outputs bins directly (0-2)
                        bin_label = pred_class
                        mst_label = pred_class
                    else:
                        # Model outputs MST 1-10 (class 0-9)
                        mst_label = pred_class + 1
                        bin_label = mst_to_bin(mst_label)
                
                elif self.model_type == "regressor":
                    pred_raw = self.model(img_tensor).squeeze().item()
                    # Regressor always outputs sigmoid [0,1] → scale to [1,10]
                    pred_scaled = pred_raw * 9 + 1
                    mst_label = int(np.clip(round(pred_scaled), 1, 10))
                    bin_label = mst_to_bin(mst_label)
    ...
    def predict_batch(self, img_pils: list) -> list:
        ...
        Returns: list of (mst_label, bin_label) tuples
        """
        if not img_pils:
            return []
        ...
        try:
            ...
            img_tensors = [self.transform(img) for img in img_pils]
            ...
            with torch.no_grad():
                if self.model_type == "classifier":
                    ...
                    for pred_class in pred_classes:
                        if self.output_mode == "binned":
                            bin_label = int(pred_class)
                            mst_label = int(pred_class)
                        else:
                            mst_label = int(pred_class) + 1
                            bin_label = mst_to_bin(mst_label)
                    ...
                    pred_raw = self.model(batch_tensor).squeeze().cpu().numpy()
