...
def balance_by_label(samples):
    ...
    balanced = []
    for label in sorted(buckets.keys()):  # Sort for determinism
        group = buckets[label]
...
# python DefineDatasetSplits_fixed.py --annotations "E:\ImageRetrieval\StableDiffusionGeneratedImages_Annotations\annotations.jsonl" --out_json "splits_gender_face_stratified_balanced.json" --label_key gender --annotation_source face --face_path_hint face --split_mode stratified --seed 42 --balance