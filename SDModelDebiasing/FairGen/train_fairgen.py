...
from src.models.fairgen_adapter import (
    FairGenNetwork,
    FairGenLayer,
)
...
def train(
    ...
    network = FairGenNetwork(
        ...
        multiplier=getattr(config.network, 'multiplier', 1.0),
    ...
    # Prepare orthogonal reference adapters if configured
    orth_refs = []
    if (
        config.train.orthogonal_regularization
        and config.train.orthogonal_weight > 0.0
        and config.train.orthogonal_reference_paths is not None
        and len(config.train.orthogonal_reference_paths) > 0
    ):
        for path in config.train.orthogonal_reference_paths:
            sd, _ = load_state_dict(path, weight_dtype)
            orth_refs.append(sd)
    ...
    for i in pbar:
        ...
        # For FairGen, positive is epsilon(X_t, d, t) and neutral is epsilon(X_t, g_t, t);
        # target_latents is prediction with adapters active under group condition.
        ...
        # Add orthogonal regularization across adapter weights if enabled
        if config.train.orthogonal_regularization and config.train.orthogonal_weight > 0.0 and len(orth_refs) > 0:
            orth_loss = 0.0
            # collect current adapter P and Q weights per layer
            current_sd = network.state_dict()
            # heuristic: interpret lora_up as P and lora_down as Q^T in 1D adapter setting
            current_ps = [v for k, v in current_sd.items() if k.endswith('.lora_up.weight')]
            current_qs = [v for k, v in current_sd.items() if k.endswith('.lora_down.weight')]

            for ref in orth_refs:
                ref_ps = [v for k, v in ref.items() if k.endswith('.lora_up.weight')]
                ref_qs = [v for k, v in ref.items() if k.endswith('.lora_down.weight')]
                # align lengths defensively
                m = min(len(current_ps), len(ref_ps))
                n = min(len(current_qs), len(ref_qs))
                if m > 0:
                    for i in range(m):
                        # project to vectors by mean over dims to approximate 1-d direction
                        p_cur = current_ps[i].view(current_ps[i].shape[0], -1)
                        p_ref = ref_ps[i].to(p_cur.device, dtype=p_cur.dtype).view(ref_ps[i].shape[0], -1)
                        # cosine-style overlap via normalized Frobenius inner product
                        p_cur_n = p_cur / (p_cur.norm(p=2) + 1e-6)
                        p_ref_n = p_ref / (p_ref.norm(p=2) + 1e-6)
                        orth_loss = orth_loss + torch.sum(p_cur_n * p_ref_n)
                if n > 0:
                    for i in range(n):
                        q_cur = current_qs[i].view(current_qs[i].shape[0], -1)
                        q_ref = ref_qs[i].to(q_cur.device, dtype=q_cur.dtype).view(ref_qs[i].shape[0], -1)
                        q_cur_n = q_cur / (q_cur.norm(p=2) + 1e-6)
                        q_ref_n = q_ref / (q_ref.norm(p=2) + 1e-6)
                        orth_loss = orth_loss + torch.sum(q_cur_n * q_ref_n)

            loss["loss"] = loss["loss"] + config.train.orthogonal_weight * orth_loss
            loss["loss/orth"] = config.train.orthogonal_weight * orth_loss
...
# python train_fairgen.py --config_file configs/gender/female/config_original.yaml
