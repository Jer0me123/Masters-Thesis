...
class PromptSettings(BaseModel):  # yaml
    ...
    # FairGen-specific parameters
    eta: float = 1.0  # strength for FairGen guidance
    fairgen_group: Optional[str] = None  # optional: explicit group prompt g_t
    fairgen_category: Optional[str] = None  # optional: explicit category prompt d
    ...
    def fill_prompts(cls, values):
        ...
        if values.get("action", "erase") == "fairgen":
            # For FairGen, allow explicit fields; otherwise reuse existing ones
            group = values.get("fairgen_group", values.get("target", ""))
            category = values.get("fairgen_category", values.get("positive", values.get("target", "")))
            values["target"] = group
            # neutral should be the group condition (g_t)
            values["neutral"] = group
            # positive should be the attribute category (d)
            values["positive"] = category
...
class PromptEmbedsPair:
    ...
    def _fairgen(
        self,
        target_latents: torch.FloatTensor,  # prediction with adapters at group condition g_t
        positive_latents: torch.FloatTensor,  # epsilon(X_t, d, t)
        neutral_latents: torch.FloatTensor,  # epsilon(X_t, g_t, t)
        **kwargs,
    ):
        # FairGen guidance target: neutral + eta * (positive - neutral)
        # print(f"Using ETA: {self.settings.eta}")
        guided = neutral_latents + self.settings.eta * (positive_latents - neutral_latents)
        fg_loss = self.loss_fn(target_latents, guided)
        losses = {
            "loss": fg_loss,
            "loss/fairgen": fg_loss,
        }
        return losses
    ...
    def loss(
        ...
        **kwargs,
        ...
        elif self.action == "fairgen":
            return self._fairgen(**kwargs)
        else:
