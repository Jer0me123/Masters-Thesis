...
#     for name, tensor in adapters[0].items():
#         if torch.is_tensor(tensor) and (name.endswith(".lora_up.weight") or name.endswith(".lora_down.weight")):
#             print("WEIGHT --->", name, "mean_abs=", tensor.abs().mean().item(), "norm=", tensor.norm().item(), "shape=", tuple(tensor.shape))
#             break
...
#                 scale = 1 #float(getattr(config, "alpha", 1.0))
                
#                 weighted = {}
#                 for adapter, w in zip(adapters, used_multipliers):
#                     for key, value in adapter.items():
#                         merged = weighted.get(key, 0) + value * w

#                         # Explicitly scale LoRA tensors
#                         if torch.is_tensor(merged) and (".lora_up." in key or ".lora_down." in key):
#                             merged = merged * scale
...
#                 print("Adapter multiplier:", scale)
...
def infer_with_fairgen(
    ...
    inv_idx = 1  # index of adapter you want to invert

    for k in adapters[inv_idx]:
        adapters[inv_idx][k] = -adapters[inv_idx][k]
    ...
    with torch.no_grad():
        for prompt in config.prompts:
            ...
            if categories and categories > 0:
                ...
                network.load_state_dict(weighted)
                # scale adapter strength if alpha provided
                print(getattr(config, 'alpha', 1.0))
                network.multiplier = getattr(config, 'alpha', 1.0)
                with network:
                    images = pipe(
                        negative_prompt=config.negative_prompt,
...
# python infer_fairgen.py --config configs/generation_gender.yaml --adapter_paths output/gender/female/gender_female_500steps.safetensors --base_model CompVis/stable-diffusion-v1-4 --precision fp32 --categories 1 --uniform

# python infer_fairgen.py --config configs/generation_gender.yaml --adapter_paths output/gender/female_original/gender_female_last.safetensors --base_model CompVis/stable-diffusion-v1-4 --precision fp32 --categories 1

# python infer_fairgen.py --config configs/generation_gender.yaml --adapter_paths output/gender/doctor_female/doctor_female_1000steps.safetensors output/gender/doctor_female/doctor_female_1000steps.safetensors --base_model CompVis/stable-diffusion-v1-4 --precision fp16 --categories 2