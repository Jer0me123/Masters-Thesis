...
# def subject_stratified_split(csv_path, val_ratio):
...
#     train_df = train_df[[filename_col, label_col]]
#     val_df   = val_df[[filename_col, label_col]]
...
#     print("[INFO] Train MST Distribution:")
#     for k in range(1, 11):
#         print(f"  MST {k}: {train_counts.get(k, 0)} images")

#     print("\n[INFO] Val MST Distribution:")
#     for k in range(1, 11):
...
#     # (This is implicit from train_test_split on person_id, but we assert anyway)
#     # If you add person_id back into dfs, you can check explicitly.
...
#     # Y
#     hists.append(
#         cv2.calcHist([img_ycc], [0], None, [bins], [0, 256]).flatten()
#     )

#     # V
#     hists.append(
#         cv2.calcHist([img_hsv], [2], None, [bins], [0, 256]).flatten()
#     )

#     # L
#     hists.append(
#         cv2.calcHist([img_lab], [0], None, [bins], [0, 256]).flatten()
#     )
...
#     X, y = [], []
#     for filename, label in tqdm(df.values, desc="Extracting features"):
...
# def hyperparam_search_clf(X_train, y_train, n_jobs=-1, n_iter=30, random_state=42):
#     """
...
# def hyperparam_search_reg(X_train, y_train, n_jobs=-1, n_iter=30, random_state=42):
#     """
...
#     train_df, val_df = subject_stratified_split(csv_path, val_ratio)
...
#         if hparam_search:
#             rf = hyperparam_search_clf(X_train, y_train, n_jobs=n_jobs,
#                                        n_iter=search_iter, random_state=random_state)
#         else:
...
#         if hparam_search:
#             rf = hyperparam_search_reg(X_train, y_train, n_jobs=n_jobs,
#                                        n_iter=search_iter, random_state=random_state)
#         else:
...
#     parser.add_argument("--csv-path", required=True, help="CSV with columns: filename,label,person_id")
#     parser.add_argument("--image-dir", required=True, help="Directory with segmented face images")
#     parser.add_argument("--model-out", default=".", help="Directory to save the trained model(s)")
#     parser.add_argument("--mode", type=str, default="clf", choices=["clf", "reg"], help="Training mode: clf=classifier, reg=regressor")
...
#     parser.add_argument("--class-weight", type=str, default="none", choices=["none", "balanced", "balanced_subsample"], help="Only used in classifier mode")
#     parser.add_argument("--bins", type=int, default=256, help="Number of histogram bins per channel (default 256 = paper)")
    
#     parser.add_argument("--hparam-search", action="store_true", help="Enable hyperparameter search with RandomizedSearchCV")
#     parser.add_argument("--search-iter", type=int, default=30, help="Number of RandomizedSearch iterations")
