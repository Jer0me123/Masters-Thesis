...
class SkinToneClassificationDataset(Dataset):
    """
    Expects df with columns:
      0: filename
      1: MST label (1-10, float or int)
      2: person_id (string/int)
    Returns:
      image_tensor, class_index (0-9, long), person_id (string)
    """
...
def build_dataloaders(csv_path, image_dir, batch_size, val_ratio,
    ...
    total_train = len(train_mst)
    class_counts = Counter(train_mst)  # keys 1..10
    ...
    class_weight_tensor = torch.zeros(10, dtype=torch.float32)
    for mst in range(1, 11):
        idx = mst - 1
        class_weight_tensor[idx] = capped_weights[mst]
    ...
    train_dataset = SkinToneClassificationDataset(train_df, image_dir, train_tf)
    val_dataset = SkinToneClassificationDataset(val_df, image_dir, val_tf)
    ...
    class_freq = Counter(train_mst)  # mst=1..10
    ...
    sampler = torch.utils.data.WeightedRandomSampler(
        weights=img_weights,
        num_samples=len(img_weights),   # ~same epoch size
...
def tone_bin_from_class_idx(class_idx):
    """
    Map MST class index 0..9 to bin:
      0..2  -> light
      3..6  -> mid
      7..9  -> dark
    Returns integer bin id {0,1,2}.
    """
    idx = int(class_idx)
    if idx <= 2:
        return 0
...
def compute_classification_metrics(preds, targets):
    ...
    # 3-bin accuracy
    preds_bin = torch.tensor([tone_bin_from_class_idx(i.item()) for i in preds])
    targets_bin = torch.tensor([tone_bin_from_class_idx(i.item()) for i in targets])
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch):
    ...
    for imgs, labels, person_ids in pbar:
        ...
        preds_bin = torch.tensor([tone_bin_from_class_idx(i.item()) for i in preds.cpu()])
        labels_bin = torch.tensor([tone_bin_from_class_idx(i.item()) for i in labels.cpu()])
...
def validate(model, loader, criterion, device, epoch):
    ...
    with torch.no_grad():
        ...
        for imgs, labels, person_ids in pbar:
            ...
            batch_metrics = compute_classification_metrics(preds, labels)
            pbar.set_postfix({
                "loss": f"{loss.item():.4f}",
    ...
    # Image-level metrics
    img_metrics = compute_classification_metrics(preds_all, targets_all)
    ...
    person_metrics = compute_classification_metrics(person_preds, person_targets)
