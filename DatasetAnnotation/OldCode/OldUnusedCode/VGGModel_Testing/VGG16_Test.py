...
#  - SHUFFLED VALIDATION + FULL DIAGNOSTIC STATS
##############################################################
...
def build_dataloaders(csv_path, image_dir, batch_size, val_ratio, lab_mean, lab_std, num_workers=4):
    ...
    # CHANGED: Shuffle validation to ensure representative batches
    val_loader = DataLoader(
        val_dataset,
        batch_size=batch_size,
        shuffle=True,  # CHANGED: Was False, now True
        num_workers=num_workers,
        pin_memory=True,
        persistent_workers=True,
        prefetch_factor=2,
...
def print_diagnostics(imgs, labels, preds, tag):
    """First batch diagnostics - shows if data/predictions are in expected range"""
    print(f"\n=== [DIAG] {tag} ===")
...
def validate(model, loader, device, epoch, threshold=0.5):
    """
    UPDATED: Now collects ALL predictions and labels to show full distribution.
    This reveals if model is collapsing to predict mostly one MST value.
    """
    model.eval()
    mse = nn.MSELoss()
    total_loss = 0.0
    ...
    # NEW: Collect ALL predictions and labels for distribution analysis
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(pbar):
            ...
            # NEW: Collect for full-dataset analysis
            ...
            if i == 0:
                print_diagnostics(imgs, labels, preds, f"VALID EPOCH {epoch} - FIRST BATCH")

    # NEW: Print FULL validation statistics
    all_preds = np.array(all_preds)
    all_labels = np.array(all_labels)
    
    # Convert back to MST scale [1,10]
    preds_mst = all_preds * 9 + 1
    labels_mst = all_labels * 9 + 1
    
    print(f"\n{'='*60}")
    print(f"[FULL VALIDATION STATS] EPOCH {epoch}")
    print(f"{'='*60}")
    print(f"Predictions MST: min={preds_mst.min():.2f}, max={preds_mst.max():.2f}, "
          f"mean={preds_mst.mean():.2f}, std={preds_mst.std():.2f}")
    print(f"Labels MST:      min={labels_mst.min():.2f}, max={labels_mst.max():.2f}, "
          f"mean={labels_mst.mean():.2f}, std={labels_mst.std():.2f}")
    
    # Show MST distribution of PREDICTIONS (rounded to nearest integer)
    pred_rounded = np.clip(np.round(preds_mst).astype(int), 1, 10)
    label_rounded = np.clip(np.round(labels_mst).astype(int), 1, 10)
    
    pred_counts = Counter(pred_rounded)
    label_counts = Counter(label_rounded)
    
    print(f"\n{'Prediction Distribution Across MST:':-^60}")
    for mst in range(1, 11):
        pred_pct = 100 * pred_counts.get(mst, 0) / len(pred_rounded)
        label_pct = 100 * label_counts.get(mst, 0) / len(label_rounded)
        bar = "█" * int(pred_pct / 2)  # Visual bar
        print(f"  MST {mst:2d}: {pred_counts.get(mst, 0):4d} preds ({pred_pct:5.1f}%) "
              f"| {label_counts.get(mst, 0):4d} labels ({label_pct:5.1f}%) {bar}")
    
    # Check for collapse
    max_pred_pct = max(pred_counts.values()) / len(pred_rounded) * 100
    if max_pred_pct > 60:
        dominant_mst = max(pred_counts, key=pred_counts.get)
        print(f"\n⚠️  WARNING: {max_pred_pct:.1f}% of predictions are MST {dominant_mst}")
        print(f"    Model may be collapsing to predict the dominant class!")
