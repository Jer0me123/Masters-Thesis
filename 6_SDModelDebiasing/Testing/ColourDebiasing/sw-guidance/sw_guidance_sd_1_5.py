...
captions = [
    "a doctor wearing a white coat in a hospital"
]
...
def decode_to_image(latent_aux):
    with torch.no_grad():
        latent_aux = latent_aux.float()              # 🔴 critical
        image = vae.decode(1 / 0.18215 * latent_aux).sample
    image = (image / 2 + 0.5).clamp(0, 1)
    image = image.detach().cpu().permute(0, 2, 3, 1).numpy()
    images = (image * 255).round().astype("uint8")
...
for idx, prompt in enumerate(tqdm(captions)):
    ...
    with torch.no_grad():
        ...
        prompt_embeddings = text_encoder(text_input.input_ids.to(device))[0].to(dtype)
    ...
    seed = 45#np.random.randint(0, 2**31 - 1)
    print(f"seed: {seed}")
    ...
    for i, j in tqdm(time_pairs):
        if i > 1:
            if j < i:
                ...
                ## DEl if not needed
                if do_guidance and i < int(scheduler.num_inference_steps*stop_guidance):
                    for _ in range(M):
                        u = u.detach()
                        ...
                        x_0 = (
                            x_hat_t.float()
                            - (1 - at).sqrt().float() * noise_pred.float()
                        ) / at.sqrt().float()
                        ...
                        image = vae.decode(1 / 0.18215 * x_0.float()).sample
                        ...
                        pixels_gen = image.squeeze(0).reshape(3, -1).float()
                        gen_mean = torch.mean(pixels_gen, dim=1)
                        gen_cov = torch.cov(pixels_gen)
                ...
                # Save intermediate image for this diffusion step
                step_img = decode_to_image(x0_pred_last.to(device))
                step_img.save(f"{save_folder}/steps/img_{idx}_step_{i:02d}.png")
