...
M = 5#0 # iterations for each denoising steps
guidance_scale = 8
u_lr = 1/25
...
DECODE_START = 0 #int(0.4 * scheduler.num_inference_steps)
print(DECODE_START)
    ...
    for i, j in tqdm(time_pairs):
        if i > 1:
            if j < i:
                ...
                if do_guidance and i < int(scheduler.num_inference_steps*stop_guidance) and i > DECODE_START:
                    for _ in range(M):
                        u = u.detach()
