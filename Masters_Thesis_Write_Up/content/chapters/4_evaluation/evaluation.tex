...
modal value: Re-LAION-5B peaks at MST 5 and MST 6 with 
approximately 28\% each, while Stable Diffusion v1.5 peaks 
...
reveals that gender skew is pervasive across all occupational 
categories in both datasets, but its magnitude and pattern 
...
agreement in their directional signal: groups identified as
female-majority in US BLS data are also
female-majority in the global statistics, while groups such
as Skilled Agricultural, Craft \& Trades, and Plant \&
Machine Operators are male-dominated in both. This convergence
between the two independent baselines strengthens the
...
Against these baselines, Re-LAION-5B falls below the real-world
female proportion in eight of ten ISCO groups, most notably
in Professionals (approximately 43\% vs.\ 50\% US BLS),
Service \& Sales (approximately 46\% vs.\ 51\% US BLS), and
Technicians \& Associate Professionals (approximately 44\%
vs.\ 49\% US BLS). The training corpus therefore does not
accurately reflect the gender composition of the workforce it
implicitly represents, already exhibiting a systematic
male-skew relative to observed labour force statistics. Stable
Diffusion v1.5 substantially widens these gaps in nearly every
group, with the most pronounced deviations occurring in
Managers (approximately 19\% vs.\ 38\% US BLS, a gap of 19
percentage points), Professionals (approximately 29\% vs.\
50\% US BLS), and Service \& Sales (approximately 37\% vs.\
51\% US BLS). The single exception is Clerical Support, where
both Re-LAION-5B and Stable Diffusion v1.5 broadly align with
the US BLS reference at approximately 60\% and 52\% female
respectively --- a case where the model's learned stereotype
coincides with real-world labour force composition, rather
than reflecting an accurate learned prior.

Taken together, these comparisons establish a two-stage bias
pattern. Re-LAION-5B under-represents women relative to
real-world labour force statistics, reflecting the gender
imbalances inherent in the web-scraped imagery from which it
is constructed. Stable Diffusion v1.5 then amplifies this
under-representation further, producing occupational gender
distributions that are more male-skewed than even the already
biased training corpus. The generative model therefore does
not merely inherit the distributional properties of its
training data but actively exacerbates them, producing
representations that diverge substantially from both
demographic parity and empirically observed occupational
gender composition.
...
(96.4\%) among the most extreme SDv1.5 outputs. The 33 professions where the datasets disagree in skew direction represent cases where the model's learned priors override the distributional 
signal in its training data. These cases warrant further 
investigation but are beyond the scope of the current analysis.
...
summarises the results for all transformations evaluated.
...
not artefacts of a single image property but are embedded coherently 
across the full representational spectrum.
...
--- over texture, colour, spatial layout, and scene composition --- 
...
gender at AUC 0.947 in the unoccluded person and AUC 0.747 from 
background context alone; the corresponding skin tone values are 
0.900 and 0.774. These figures confirm that the generative model 
...
exploited by a linear classifier. Re-LAION-5B shows no such 
...
tone of the depicted individual. The dataset classification 
experiments cannot reveal which class is responsible for the 
...
the lowest above-chance values recorded for any geometric 
probe. The contrast between the high dataset classification 
...
primarily compositional --- camera distance, scene depth 
layering, background structure --- rather than body-geometric, 
...
Low-level spurious signal is eliminated across all non-person representations. Mean RGB
classification falls from 0.579 on the standard SDv1.5 corpus to 0.507 on the debiased
...
The background-only probe (MaskRectNoBg) achieves AUC 0.701 on the debiased corpus.
Direct comparison with the standard SDv1.5 value of 0.587 is complicated by the
different label distributions: the 0.587 value reflected near-complete majority-class
collapse in a strongly male-skewed corpus, whereas the debiased corpus with balanced
50/50 labels requires genuine discrimination. The 0.701 value indicates that some
background-level signal persists, likely because ITI-GEN's conditioning of scene content
and the ControlNet pose constraint together influence global composition in ways that
partly correlate with the gender conditioning token. Nonetheless, the background-only AUC
of 0.701 is substantially lower than the person-level probes (0.985--0.999), confirming
that the primary carrier of gender signal in the debiased corpus is the person's
...
All remaining non-person probes collapse to at or near chance. Object detection falls
from 0.663 to 0.439 (unrestricted) and from 0.626 to 0.461 (restricted), both at or near chance, indicating that the debiased corpus exhibits no systematic skin-tone-correlated
object co-occurrence patterns. Depth at pose keypoints falls from 0.655 to 0.443 and pose
keypoints from 0.666 to 0.483, both at or near chance, confirming that geometric representations
retain no skin tone signal. Caption probing falls from 0.673 to 0.490, approaching chance, indicating that
the semantic content of generated descriptions is no longer systematically correlated with
the skin tone conditioning token.

The occlusion probes show consistent reductions across all variants. MaskRectNoBg falls
from 0.660 to 0.543, close to chance, while MaskRect falls from 0.728 to 0.632,
indicating a partial but incomplete elimination of background-level skin tone signal. The
retention of above-chance signal in background-only probes suggests that the scene
composition generated by the model retains some skin-tone-correlated properties even
under pose conditioning, consistent with the background entanglement observed in the
gender analysis.
...
The debiasing evaluation is restricted to a single 
profession and a single combination of inference-time 
interventions. It does not evaluate weight editing 
or fine-tuning approaches, and results may not 
generalise to other professions, prompts, or model 
architectures. The probing results demonstrate that 
low-level spurious correlations are eliminated at the 
representation level for the doctor profession, but 
whether this elimination generalises across the full 
200-profession set or persists under different prompt 
formulations remains untested. The residual 
background-level signal identified in both the gender 
and skin tone analyses indicates that background-level 
demographic conditioning remains partially entangled 
with the generation process, and constitutes the 
primary open question for future work on this pipeline.
...
The demographic distribution analysis reveals that Stable Diffusion v1.5 does not merely
reflect the imbalances present in its training corpus but actively amplifies them. Gender
amplification is confirmed in 178 of 200 professions, with the female proportion falling
from 41.5\% in Re-LAION-5B to 29.02\% in Stable Diffusion v1.5. The skin tone analysis
reveals a near-complete collapse of dark-toned representation in generative outputs: the
Dark-bin proportion falls from 0.81\% in Re-LAION-5B to 0.14\% in Stable Diffusion v1.5,
an 83\% reduction, while the Light-bin proportion almost doubles from 12.45\% to 22.87\%.
Skin tone amplification toward lighter tones is confirmed in 175 of 200 professions.

The demographic probing experiments reveal qualitatively distinct spurious feature
profiles between the two datasets. In Re-LAION-5B, gender and skin tone information is
localised almost entirely within the visible appearance of the depicted person: every
occlusion variant collapses performance to chance, and residual signal from colour
statistics, object co-occurrence, and skeletal geometry is modest. In Stable Diffusion
v1.5, demographic information is distributed across the full image and is recoverable from
background context alone, from the person silhouette independently of background, and from
natural language descriptions of generated content. This cross-modal persistence
demonstrates that the stereotyping embedded in the generative model is coherent and
pervasive rather than confined to low-level pixel statistics.
