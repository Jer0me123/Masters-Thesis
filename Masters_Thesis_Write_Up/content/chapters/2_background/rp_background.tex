...
filtering~\cite{Laion_2024_Relaion5B}; LAION-Aesthetics v2 5+, a quality-filtered
offshoot of LAION-2B-en, constitutes the training corpus for
SDv1.5~\cite{Schuhmann_2022_LaionAesthetics}. SDv1.5 is a
...
increase prompt adherence at the cost of stochastic diversity~\cite{Ho_2022_CFG}. Its
fully documented training lineage makes it uniquely suited for mechanistic bias auditing,
unlike later models such as SDXL~\cite{Podell_2023_SDXL} that rely on proprietary
corpora. Zeng et al.~\cite{Zeng_2024_UnderstandingBiasinLargeScaleVisualDatasets} operationalise spurious
...
image captioning, preserves a targeted visual property while suppressing others,
revealing that dataset identity is redundantly encoded across colour, texture, geometry,
and semantic content simultaneously and that generative models can preserve or amplify training-data biases in their outputs.
Meister et al.~\cite{Meister_2023_GenderArtifactsinVisualDatasets} provide a complementary
occlusion-based probing protocol, establishing that gender
artefacts are distributed across the entire image structure rather than localised to the
...
Meister et al.'s occlusion protocol to both retrieved and generated corpora, extending
it to skin tone as a second demographic dimension beyond Meister et al.'s gender-only
scope.
...
Chamfer-based colour alignment~\cite{Shum_2025_ColorAlignment} to steer colour-level
spurious priors.

% OLD BACKGROUND & LITERATURE REVIEW
% \chapter{Background \& Literature Review}
