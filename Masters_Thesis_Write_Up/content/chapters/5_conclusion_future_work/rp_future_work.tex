...
SDv1.5 with LCM-LoRA acceleration, both filtered through analogous structural validation
pipelines. Demographic annotation was performed using the Realistic Gender
...
beyond the coarser scales used in prior work. Addressing Objective~2, the
...
et al.\ revealed a fundamental asymmetry between datasets: Re-LAION-5B exhibits
demographic distributions that diverge from real-world labour force composition, with
demographic signal absent from background context but present modestly across colour,
geometry, and semantic representations. In contrast, SDv1.5 exhibits a fundamentally
different structure: demographic information is distributed across scene context and
background, recoverable from background context alone (gender AUC 0.747; skin tone AUC
0.728), from person silhouette geometry (gender AUC 0.834), and from natural language
descriptions (gender AUC 0.771). Gender amplification was confirmed in 178 of 200
professions and skin tone amplification toward lighter tones in 175 of 200
...
prior at the distributional level, achieving a uniform demographic distribution by
construction with most non-person probes collapsing to at or near chance for both gender
and skin tone. Residual occlusion signal persists, strongest when background is retained (gender
MaskRect AUC 0.995; skin tone MaskRect AUC 0.632) but also detectable under MaskRectNoBg
(gender AUC 0.701; skin tone AUC 0.543), indicating that scene composition and coarse
person-region structure remain partially entangled with the demographic conditioning
signal even under pose and appearance control. Addressing Objective~5, the
combined evidence characterises a two-stage bias
propagation-and-transformation process in which Re-LAION-5B
exhibits demographic distributions that diverge from real-world labour force composition
with no independent background-level demographic signal, and SDv1.5 then amplifies these
imbalances substantially while transforming their distribution, projecting demographic
priors beyond the person onto background, scene composition, object layout, and textual
description. Inference-time debiasing can measurably reduce spurious correlations at the
representation level without model retraining, but background-level entanglement
represents the primary remaining challenge.
...
full-person/no-background ambiguity reflects masking limits or probe capacity; and extending the
...
% OLD CONCLUSIONS AND FUTURE WORK
% \chapter{Conclusions and Future Work}
...
% encoded across colour, texture, geometry, semantics, and language simultaneously.
% Addressing Objective~3, demographic probing following the occlusion framework of Meister
% et al.\ revealed a fundamental asymmetry between datasets: in Re-LAION-5B, demographic signal appears to be predominantly localised within person appearance, with background context contributing negligibly; however, the results do not permit a definitive conclusion that person-level features alone are sufficient. In contrast, SD v1.5 exhibits a fundamentally different structure: demographic information is distributed across scene context and background, recoverable from background context alone (gender AUC 0.747; skin tone AUC 0.728), from person silhouette geometry (gender AUC 0.834), and from natural language descriptions (gender AUC 0.771). Gender amplification was confirmed in 178 of 200 professions and skin tone amplification toward lighter tones in 175 of 200
% professions. Addressing Objective~4, the combined ITI-GEN, ControlNet OpenPose, and
% Chamfer colour alignment pipeline successfully overrides SDv1.5's strongly skewed learned
% prior, achieving a uniform demographic distribution by construction with most non-person
% probes collapsing to at or near chance for both gender and skin tone. Residual
