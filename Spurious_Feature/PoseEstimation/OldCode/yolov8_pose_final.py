...
def collect_image_paths(input_dir, IMG_EXTS, processed, show_progress=True):
    IMG_EXTS = set(e.lower() for e in IMG_EXTS)
    ...
    try:
        while stack:
            ...
            with os.scandir(current) as it:
                for entry in it:
                    # Count every filesystem entry once
                    pbar.update(1)

                    if entry.is_dir(follow_symlinks=False):
                        if "facemesh" not in entry.name.lower():
...
def process_directory(
    input_dir, output_dir, model_size, device,
    ...
    paths = collect_image_paths(input_dir, IMG_EXTS, processed)


    loader = FastImageLoader(num_workers)
    model = YOLOv8PoseEstimator(model_size, device)
